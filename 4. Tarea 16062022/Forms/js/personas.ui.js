...
function mostrarPersonas()
  {
    ...
    if (personas!=null)
    {
    salida="<h3>Personas <a id='btNuevo'data-id='-1' class='btn btn-outline-secondary ' >Nuevo</a> </h3>";
    for (var i = 0; i < personas.length; i++) {
        console.log("girando");
        nombredeCiudad=getCiudadNameById(personas[i].ciudad_id);
        salida=salida+"<div class='card'><div class='card-header'>"+personas[i].apellido+", "+personas[i].nombre+"</div><div class='card-body'><div class='row'><div class='col'><p class='card-text'><label>C.I: </label>"+personas[i].cin+ "</p><p class=''><label>Fecha de Nacimiento: </label>" +personas[i].fenac+"</p><p class='card-text'><label>Localidad: </label>"+nombredeCiudad+"</p></div><div class='col'><a data-id='"+personas[i].id+"'  class='btn btn-outline-secondary btn-editPersona'>Editar</a><a data-id="+personas[i].id+" ''  class='btn btn-outline-danger btn-borrarPersona'>Borrar</a></div></div></div></div>";
