...
if (isset($_POST['enviar'])) {
 ...
 if($_POST['cin']==""){
  $error++;
  array_push($errores, "El nº de Cédula de Identidad no puede estar vacío.");
}
 if($_POST['nombre']==""){
  $error++;
...
if($_POST['fenac']==""){
  $error++;
  array_push($errores, "La fecha no puede estar vacía.");
}
 if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
     $error++;
     array_push($errores,"El email debe ser válido ");
 }
    if($error>0){
